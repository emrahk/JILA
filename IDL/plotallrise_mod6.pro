pro plotallrise_mod6, inpstr, inx, radonly=radonly, $
...
;
;
;MODIFIED FROM plotalldecay_mod5.pro, this adds upper limit for diskbb
;
; LOGS
; 
;
...
IF aa[0] ne -1 THEN BEGIN
   ...
   IF aaok[0] NE -1 THEN oploterror,time[aa[aaok]],tin[0,aa[aaok]],tin[1,aa[aaok]],psym=5,/nohat,color=colcode[0],errcol=colcode[0],syms=1.3,thick=1.5
   aaup=where(tin[1,aa] EQ -1)
   IF aaup[0] NE -1 THEN oplot,time[aa[aaup]],tin[0,aa[aaup]],psym=5,color=colcode[0],syms=1.3,thick=1.5
ENDIF


IF bb[0] ne -1 THEN BEGIN
   bbok=where(tin[1,bb] NE -1)
   IF bbok[0] NE -1 THEN oploterror,time[bb[bbok]],tin[0,bb[bbok]],tin[1,bb[bbok]],psym=5,/nohat,color=colcode[1],errcol=colcode[1],syms=1.3,thick=1.5
   bbup=where(tin[1,bb] EQ -1)
   IF bbup[0] NE -1 THEN oplot,time[bb[bbup]],tin[0,bb[bbup]],psym=5,color=colcode[1],syms=1.3,thick=1.5
ENDIF
...
IF bolcor THEN BEGIN
   ...
   IF aa[0] ne -1 THEN BEGIN
      ...
      aaok=where(outdelf[1,aa] NE -1)
      oploterror,time[aa[aaok]],outdelf[0,aa[aaok]],outdelf[1,aa[aaok]],psym=5,color=colcode[0],syms=1.3,thick=1.5,/nohat,errcol=colcode[0]
      aaup=where(outdelf[1,aa] EQ -1)
      IF aaup[0] NE -1 THEN BEGIN
         FOR ix=0,n_elements(aaup)-1 DO BEGIN
            IF outdelf[0,aa[aaup[ix]]] GT yr[0] THEN arrow,time[aa[aaup[ix]]],outdelf[0,aa[aaup[ix]]], time[aa[aaup[ix]]], yr[0], /data, color=colcode[0]
            ENDFOR
      ENDIF
   ENDIF
   ...
   IF bb[0] ne -1 THEN BEGIN
      ...
      IF bbok[0] NE -1 THEN  oploterror,time[bb[bbok]],outdelf[0,bb[bbok]],outdelf[1,bb[bbok]],psym=5,color=colcode[1],syms=1.3,thick=1.5,/nohat,errcol=colcode[1]
      bbup=where(outdelf[1,bb] EQ -1)
      IF bbup[0] NE -1 THEN BEGIN
         FOR ix=0,n_elements(bbup)-1 DO BEGIN
            IF outdelf[0,bb[bbup[ix]]] GT yr[0] THEN arrow,time[bb[bbup[ix]]],outdelf[0,bb[bbup[ix]]], time[bb[bbup[ix]]], yr[0], /data, color=colcode[1]
            ENDFOR
      ENDIF
...
IF (aa[0] NE -1) THEN BEGIN
   IF (aaok[0] NE -1) THEN oploterror,time[aa[aaok]],$
                                    rin[0,aa[aaok]], rin[1,aa[aaok]], psym=5,$
                                     syms=1.3,thick=1.5,$
                                    /nohat,color=colcode[0], errcol=colcode[0]
ENDIF

IF (bb[0] NE -1) THEN BEGIN
   IF (bbok[0] NE -1) THEN oploterror,time[bb[bbok]],$
                                 rin[0,bb[bbok]],$
                                 rin[1,bb[bbok]],psym=5,syms=1.3,thick=1.5,$
                                 /nohat,color=colcode[1],errcol=colcode[1]
ENDIF
