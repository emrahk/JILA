pro getoirdata_gx05, oirinfo, name, year
...
; LOGS
;
; fluxes added to structure, bug fix assigning I values to V 
;
...
dt=where((MJD GT 53045) AND (MJD LE 53270.))
nod=n_elements(dt)
print,nod*4.,'number of OIR elements'

;oirinfo=create_struct('name',name,'year',year,'dates',fltarr(nod*4),'mag',fltarr(2,nod*4),'flux',fltarr(2,nod*4),'band',strarr(nod*4),'inst','SMARTS')
