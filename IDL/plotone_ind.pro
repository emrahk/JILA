pro plotone_ind, inpstr, inx, usecol=usecol, pcaonly=pcaonly, $
...
IF (ps AND NOT keyword_set(fname)) THEN fname='indsingle.eps'
...
IF dd[0] NE -1 THEN oploterror,time[dd],ind[0,dd],ind[1,dd],psym=psm,$
/nohat,color=colcode[3],errcol=colcode[3],syms=symsz,thick=thk

IF ee[0] NE -1 THEN oploterror,time[ee],ind[0,ee],ind[1,ee],psym=psm,$
/nohat,color=colcode[3],errcol=colcode[3],syms=symsz,thick=thk
